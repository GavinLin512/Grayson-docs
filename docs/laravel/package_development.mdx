...
```
#### 3. 在 `composer.json` 設定 `autoload` 資料夾為 `src`，然後在 Terminal 輸入 `composer dump-autoload`。
:::info
...
| 400 Bad Request | 此套件的 name 及/或 版本是無效的，或是已存在與此套件相同 name 及版本的套件. |
